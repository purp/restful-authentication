...
<pre><code>  !! important: if you upgrade your site, existing user account !!
...
* "Extensions, Addons and Alternatives":http://github.com/technoweenie/restful-authentication/wikis/addons such as HAML templates
* "Security Design Patterns":http://github.com/technoweenie/restful-authentication/wikis/security-patterns with "snazzy diagram":http://github.com/technoweenie/restful-authentication/tree/master/notes/SecurityFramework.png
* "Authentication":http://github.com/technoweenie/restful-authentication/wikis/Authentication -- Lets a visitor identify herself (and lay claim to her corresponding Roles and measure of Trust)
* "Trust Metrics":http://github.com/technoweenie/restful-authentication/wikis/Trustification -- Confidence we can rely on the outcomes of this visitor's actions.
* "Authorization":http://github.com/technoweenie/restful-authentication/wikis/Authorization and Policy -- Based on trust and identity, what actions may this visitor perform?
* "Access Control":http://github.com/technoweenie/restful-authentication/wikis/Access Control -- How the Authorization policy is actually enforced in your code (A: hopefully without turning it into a spaghetti of if thens)
* "Rails Plugins":http://github.com/technoweenie/restful-authentication/wikis/Rails Plugins for Authentication, Trust, Authorization and Access Control
* "Tradeoffs":http://github.com/technoweenie/restful-authentication/wikis/Tradeoffs -- for the paranoid or the curious, a rundown of tradeoffs made in the code
* "CHANGELOG":http://github.com/technoweenie/restful-authentication/wikis/CHANGELOG -- Summary of changes to internals
* "TODO":http://github.com/technoweenie/restful-authentication/wikis/TODO -- Ideas for how you can help

These best version of the release notes are in the @notes/@ directory in the "source code":restful-authentication/tree/master -- look there for the latest version. The wiki versions are taken (manually) from there.
...
h3. Modularized to Match Security Design Patterns
...
The new password encryption (using a site key salt and stretching) will break existing user accounts' passwords. We recommend you use the --old-passwords option or write a migration tool and submit it as a patch. See the "Tradeoffs":http://github.com/technoweenie/restful-authentication/wikis/tradeoffs note for more information.
...
* %--stateful%: Builds in support for acts_as_state_machine and generates activation code. (@--stateful@ implies @--include-activation@). Based on the "VaporBase Stateful Authentication post":http://www.vaporbase.com/postings/stateful_authentication . Passing @--skip-migration@ will skip the user migration, and@--skip-routes@ will skip resource generation %--% both useful if you've already run this generator. (Needs the "acts_as_state_machine plugin":http://elitists.textdriven.com/svn/plugins/acts_as_state_machine/, but new installs should probably run with @--aasm@ instead.)
...
* %--old-passwords%: Use the older password scheme (see "Non-Backwards Compatible Changes":#COMPATIBILITY, above)
...
<pre><code>map.signup  '/signup', :controller => 'users',   :action => 'new'
...
<pre><code>map.activate '/activate/:activation_code', :controller => 'users', :action => 'activate', :activation_code => nil
</code></pre>
